<template>
	<div class="lists">
		<div class="showbtn" style="cursor:pointer;" v-if="orderInfo.systemSource == 1 || orderInfo.systemSource == 2">
			<i :class="{'el-icon-d-arrow-right': showbtn, 'el-icon-d-arrow-left': !showbtn}" @click="showbtn = !showbtn"></i>
		</div>
		<div class="details_left" :style="'width:' + (showbtn && (orderInfo.systemSource == 1 || orderInfo.systemSource == 2) ? '35%' : '0')">
			<h4 class="title_h4">订单详情</h4>
			<!-- <div class="imgs">
				<img src="./../2.png" />
			</div> -->
			<div class="orderDetial" style="box-shadow: none; position: initial;height: 100%;">
		      <!-- <h4><span class="fr" @click="orderVisable = false"><i class="icons closedico"></i></span>订单信息</h4> -->
		      <div class="odcontent" style="padding:0 30px;">
		        <h5>订单信息</h5>
		        <el-row>
		          <el-col :span="12"><label>订单号：</label>{{ orderInfo.code }}</el-col>
		        ...
		        <h5>附件</h5>
		        <div class="filesPic" v-if="orderInfo.projectFiles.length >= 1">
		          <div class="imgParent"><!--https://ww3.sinaimg.cn/mw690/6f26315cgw1ejbg4tg69xj21kw11xtwz.jpg-->
		            <!-- <div class="xx" :style="'background:url(' + url + 'cloud/project/downloadOrderFile?fileDownUrl=' + orderInfo.projectFiles[0].fileDownUrl + '&token=' + token + ') no-repeat; width:' + imgWidth + ';height:' + imgHeight + '; transform: rotate(' + rotate + 'deg); background-size:100% auto;'" @mousedown="move"></div> -->
		            <div class="xx" :style="'background:url(' + src + orderInfo.projectFiles[picindex].fileDownUrl + '&token=' + uploadHeader.token + ') no-repeat; width:' + imgWidth + ';height:' + imgHeight + '; transform: rotate(' + rotate + 'deg); background-size:contain;'" @mousedown="move"></div>
		          </div>
		...
		<div class="project_right" :style="'width:' + (showbtn && (orderInfo.systemSource == 1 || orderInfo.systemSource == 2) ? '63%' : '98%')">
			<h4 class="title_h4">创建项目</h4>
			<el-steps :active="activeCheck" finish-status="success" align-center>
				<el-step @click.native="activeCheck = canNext === false ? 0 : index" style="cursor: pointer" :title="stepse.stepName" v-for="(stepse, index) in itemList" :key="index"></el-step>
			</el-steps>
			<div class="table-data" v-loading="loading" element-loading-text="拼命加载中" style="padding-bottom:40px;">
				<el-form :model="formData" :rules="rules" ref="formData" label-width="160px">
					<el-form-item label="检测项目：">
						<el-col :span="20">
							<!--<el-select v-model="formData.code" clearable placeholder="请选择" :disabled="selectOnly" @change="selectChange" style="width:100%;">
              ...
              <el-input type="text" v-model="formData.businessName" disabled  style="width:100%;"></el-input>
					...
					<div class="squerItmes" v-for="(squerItmes, index) in itemList" v-if="index === activeCheck" :key="index" style="position:relative;">
						...
						<el-form-item v-for="(itemer, itemerKey) in squerItmes.blockFields" :label="itemer.label + '：'" v-show="itemer.type !== 'hidden'" :key="itemerKey">
							<!-- 姓名 -->
							...
							<!-- 年龄 -->
							<el-col :span="24" v-if="itemer.element === 'input' && itemer.type === 'text' && itemer.name == 'sourceOrderNo'" style="position:relative; z-index:1;">
								<!-- 订单号 -->
								<el-col :span="20">
									<el-input type="text" v-model="formData.dynamicInfo[itemer.name]" style="width:100%;"></el-input>
								</el-col>
								...
								<el-col :span="24" style="margin: 1em 0 -1em 0;">
									<el-upload class="noup" :action="imageUploadUrl" list-type="picture-card" :file-list="fileListe" :on-preview="handlePictureCardPreview" :headers="uploadHeader">
							...
							<el-col :span="4" v-if="itemer.element === 'input' && itemer.type === 'autocomplete'">
								<el-autocomplete clearable class="inline-input" v-model="formData.dynamicInfo[itemer.name]" :fetch-suggestions="(queryString, cb)=> autocompleteList(queryString, cb, itemer.name)" placeholder="请选择" :trigger-on-focus="true" @select="(item)=>autocompleteSelect(item, itemer.name)">
							...
							<el-col :span="4" v-if="itemer.element === 'input' && itemer.type === 'file'">
								<el-upload :data="updata" :action="imgUploadUrl" :on-remove="handleRemove" :before-upload="beforeUpload" :on-success="handleSuccess" :headers="uploadHeader" :file-list="fileList">
							...
							<el-col :span="20" v-if="itemer.element === 'select' && itemer.name === 'sampleType'">
								<el-select v-model="formData.dynamicInfo[itemer.name]" clearable placeholder="请选择" style="width:100%;" multiple>
							...
							<el-col :span="20" v-if="itemer.element === 'select' && itemer.name === 'batchNumber'">
								<el-select v-model="formData.dynamicInfo[itemer.name]" @change="(val)=>isValueNull(val)" placeholder="选择批次号批次号" clearable style="width:100%;">
							...
							<el-col :span="20" v-if="itemer.element === 'select' && itemer.name !== 'sampleType' && itemer.name !== 'batchNumber'">
								<el-select v-model="formData.dynamicInfo[itemer.name]" @change="seleisNull(formData.dynamicInfo[itemer.name])" clearable placeholder="请选择" style="width:100%;">
									<el-option :label="item.label" :value="item.value" v-for="(item, index) in itemer.initValueJsonObject" :key="index"></el-option>
								</el-select>
							</el-col>
							<!-- 男/女 -->
							<el-col :span="20" v-if="itemer.element === 'radio' && itemer.name === 'gender'">
							...
							<el-col :span="20" v-if="itemer.element === 'textarea' || itemer.name === 'address'">
								<el-input type="textarea" :rows="2" v-model="formData.dynamicInfo[itemer.name]" style="width:100%;"></el-input>
							</el-col>
							...
							<el-col :span="20" v-if="itemer.element === 'list' && itemer.name !== 'quality_control_result' && itemer.name !== 'gene_msi' && itemer.name !== 'gene_mmr' && itemer.name !== 'gene_mutation' && itemer.name !== 'gene_copy' && itemer.name !== 'gene_expression' && itemer.name !== 'gene_amplification' && itemer.name !== 'gene_tumor_burden' && itemer.name !== 'gene_fusion' && itemer.name !== 'dna_quality_Inspection' && itemer.name !== 'rna_quality_Inspection' && itemer.name !== 'medication_history'" style="position:relative;">
								...
								<el-table :data="formData.dynamicInfo[itemer.name]" class="itemBorder" style="width: 100%">
									<el-table-column v-for="(colums, keys, index) in itemer.initValueJsonObject[0]" :prop="keys" :label="itemer.itemJsonValue[index].name" :key="keys">
							...
							<!--dna/rna质检-->
							<el-col :span="24" v-if="itemer.name === 'dna_quality_Inspection' || itemer.name === 'rna_quality_Inspection'" style="position:relative">
								<h2 class="squerItmes_h2">{{ '　' }}</h2>
								<el-table :data="formData.dynamicInfo[itemer.name]" class="itemBorder" style="width: 100%">
						...
						<el-col :span="20" v-if="itemer.element === 'list' && itemer.name === 'gene_mmr'" style="position:relative;">
							...
							<el-col :span="20" v-if="itemer.element === 'list' && itemer.name === 'gene_expression'" style="position:relative;">
								<h4 class="squerItmes_h2">{{ itemer.label }}</h4>
								<el-button type="primary" size="mini" class="position_absolute" @click="objDataAdd(itemer.name)">+</el-button>
								<el-table :data="formData.dynamicInfo[itemer.name]" class="itemBorder" style="width: 100%">
									<el-table-column v-for="(colums, keys, index) in itemer.initValueJsonObject[0]" :prop="keys" :label="itemer.itemJsonValue[index].name" :key="index">
										<template slot-scope="scope">
							...
							<el-col :span="20" v-if="itemer.element === 'list' && itemer.name === 'quality_control_result'" style="position:relative;">
								<h4 class="squerItmes_h2">{{ itemer.label }}</h4>
								<el-button type="warning" size="mini" class="position_absolute" style="right:45px" @click="showDialog(itemer.name, 'fusion', itemer.label)">批量导入<i class="el-icon-download"></i></el-button>
								<el-button type="primary" size="mini" class="position_absolute" @click="objDataAdd(itemer.name)">+</el-button>
                  ...
                  <el-button size="small" type="warning" :disabled="uploading">导入excel</el-button>
                        ...
                        <el-option :label="'融合' + item.label" :value="'融合' + item.value" v-for="(item, indexeeee) in yinyang" :key="indexeeee"></el-option>
							...
							<el-col :span="20" v-if="itemer.element === 'list' && itemer.name === 'gene_copy'" style="position:relative;">
								<h4 class="squerItmes_h2">{{ itemer.label }}</h4>
								<el-button type="warning" size="mini" class="position_absolute" style="right:45px" @click="showDialog('copy', itemer.name, 'copy')">批量导入<i class="el-icon-download"></i></el-button>
								<el-button type="primary" size="mini" class="position_absolute" @click="objDataAdd(itemer.name)">+</el-button>
								<el-table :data="formData.dynamicInfo[itemer.name]" class="itemBorder" style="width: 100%">
									<el-table-column v-for="(colums, keys, index) in itemer.initValueJsonObject[0]" :prop="keys" :label="itemer.itemJsonValue[index].name" :key="index">
										<template slot-scope="scope">
							...
							<el-col :span="20" v-if="itemer.element === 'list' && itemer.name === 'gene_fusion'" style="position:relative;">
								<h4 class="squerItmes_h2">{{ itemer.label }}</h4>
								<el-button type="warning" size="mini" class="position_absolute" style="right:45px" @click="showDialog('fusion', itemer.name, 'fusion', itemer.label)">批量导入<i class="el-icon-download"></i></el-button>
								<el-button type="primary" size="mini" class="position_absolute" @click="objDataAdd(itemer.name)">+</el-button>
								<el-table :data="formData.dynamicInfo[itemer.name]" class="itemBorder" style="width: 100%">
									<el-table-column v-for="(colums, keys, index) in itemer.initValueJsonObject[0]" :prop="keys" :label="itemer.itemJsonValue[index].name" :key="index"> <template slot-scope="scope">
                ...
                <el-upload
                  ...
                  <el-button size="small" type="warning" :disabled="uploading">导入excel</el-button>
							...
							<el-col :span="20" v-if="itemer.element === 'list' && itemer.name === 'gene_amplification'" style="position:relative;">
								<h4 class="squerItmes_h2">{{ itemer.label }}</h4>
								<el-button type="warning" size="mini" class="position_absolute" style="right:45px" @click="showDialog('amplification', itemer.name, 'copy', itemer.label)">批量导入<i
							...
							<el-col :span="20" v-if="itemer.element === 'list' && itemer.name === 'medication_history'" style="position:relative;">
								...
								<el-table :data="formData.dynamicInfo[itemer.name]" class="tableCellFix itemBorder" style="width: 100%">
									<el-table-column v-for="(colums, keys, index) in itemer.initValueJsonObject[0]" :prop="keys" :label="itemer.itemJsonValue[index].name" :key="keys">
										<template slot-scope="scope">
											<el-select filterable remote multiple clearable v-if="index === 0"
						                        ...
						                        <el-option
						                          ...
						                          :key="indexee"
						                          :label="(item.drugName == '' ? item.drugEnName : item.drugName)"
						                          :value="item.id">
						                        </el-option>
						                    </el-select>
					...
					<el-form-item>
						...
						<el-button type="primary" @click="stepPrev" v-if="activeCheck > 0">上一步</el-button>
						<el-button type="primary" :loading="isLoading" @click="sub('formData')" v-if="activeCheck === itemList.length - 1">
							{{ isLoading ? '正在提交' : '提交' }}
						</el-button>
						<el-button type="primary" @click="stepNext" style="margin-left:10px;" :disabled="!canNext" v-if="activeCheck < itemList.length - 1">下一步</el-button>
					</el-form-item>
				</el-form>
			</div>
		...
		<el-dialog title="订单查询" :visible.sync="getSelectOrderDialog" width="80%">
			...
			<pager class="mmpPager" :current-page="getSelectFrom.currentPage" :pageSize="getSelectFrom.pageSize" :total-count="getSelectFrom.total" v-on:handleCurrentChange="(val)=>showreportList(val)"></pager>
		</el-dialog>
		...
		<!-- <div class="pdf active" v-if="pcode == '19' && isshowPdf" v-drag style="top:180px; position: absolute; right:60px; width:50%; height:calc(100% - 260px); overflow:auto; z-index:9999; border:solid 1px #ddd;">
...
<script>
	...
	export default {
      ...
      this.getDrugList()
			...
			this.serviceCreatType = localStorage.getItem('serviceCreatType')
		},
		data() {
			return {
				serviceCreatType: '',
      ...
      selectChange() {
        ...
        axios.get(url, {
          params: {
            ...
            /*that.formData.dynamicInfo['medication_history'][0].medicationDrug = [7,10]*/
           /* console.log("test="+JSON.stringify(that.formData.dynamicInfo['medication_history']))
            console.log(JSON.stringify(that.formData.dynamicInfo['medication_history'][0].medicationDrug))*/
			...
			sub(formName) {
				...
				that.$refs[formName].validate((valid) => {
					if (valid) {
						...
						axios.post(URL.api_name + 'cloud/modify_project', that.formData).then((res) => {
			...
			getFromValue() {
				...
				axios.get(URL.api_name + 'cloud/get_dynamic', {
					params: {
						...
						projectCode: that.$route.params.pcode
					}
				}).then(function(res) {
					if (res.data.code === '100') {
						...
						if (that.$route.params.productCode == '19' || that.$route.params.productCode == '41' || that.$route.params.productCode == '444') {
							that.fileList = []
							that.fileList.push({
								name: res.data.data.data.fileName,
								url: res.data.data.data.fileDownUrl
							})
						}
			...
			handleSuccess(response, file, fileList, code) {
				...
				if (response.code === '100') {
					if(that.$route.params.productCode == '19' || that.$route.params.productCode == '444') {
						console.log(JSON.parse(response.data))
						that.formData.dynamicInfo = JSON.parse(response.data)
						that.formData.dynamicInfoDto = JSON.parse(response.data)
						that.isshowPdf = true
						// console.log()
						that.pdfUrl = URL.api_name + 'cloud/project/viewProgramDiagramFile?fileUrl=' + that.formData.dynamicInfo.fileDownUrl + '&token=' + that.uploadHeader.token
			...
			objDataAdd(partName) {
				let that = this
				let xx = {}
				if(partName === 'medication_history') {
					xx = {
						medicationDrug: [],
				...
				if (that.formData.dynamicInfo[partName] != undefined) {
					that.formData.dynamicInfo[partName].push(xx)
				} else {
					that.formData.dynamicInfo[partName] = []
					that.formData.dynamicInfoDto[partName]
					that.formData.dynamicInfo[partName].push(xx)
			...
			checkListItem(item) {
				...
				that.formData.dynamicInfoDto['age'] = JSON.parse(item.applyInfoJson).applyAge
		    ...
		    getOrderDetial() {
		    	...
		    	axios.get(URL.api_name + 'cloud/getOrderProjectFile', {
		          params: {
		            projectCode: that.formData.projectCode
		          }
		        }).then(function(res){
		            if ( res.data.code === '100' ) {
		              // orderDetial
                  ...
                  console.log("jsonValue="+ that.orderInfo.jsonValue.inspectionDepartment)
		            } else {
		              that.$message({
		                type: 'error',
...
<style lang="less" scoped>
	...
	.details_left {
		...
		vertical-align: top;
		border-right: 18px solid #f7f8fb;
		transition: width .5s;
		position: relative;
		z-index: 1;
		overflow-x: hidden;
