<template>
	<div class="list">
		<div class="contentTitle pdlr20">审核血液知识库</div>
		...
		<div class="table-data" v-loading="loading" element-loading-text="拼命加载中">
			<el-table :data="tableData"  style="width: 100%" stripe>
				...
				<el-table-column prop="drug" label="药物" width="350">
        ...
        <el-table-column prop="subType" label="subtype"></el-table-column>
				<el-table-column prop="createTime" :formatter="dateFormat" v-show="status === 1" label="时间" width="160"></el-table-column>
	...
	export default {
		...
		methods: {
			...
			getListData(num, status) {
				...
				let url = ''
				url = status !== 0 ? 'cloud/project/getBacklogSummaryPage' : 'cloud/project/getBloodSummaryPage'
				that.loading = true
			...
			add() {
				this.$router.push({
					path: '/ExamineBloodNew' + '/' + this.currentPage+"/0"
				})
			},
			goinfo(code) {
				let that = this
				...
				this.$router.push({
					path: '/ExamineBloodInfo/' + code + '/' + this.currentPage + '/' + n
				})
			},
			goDetail(code) {
				// this.$router.push({
