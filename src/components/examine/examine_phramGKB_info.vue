			...
			<table>
				...
				<tr>
					...
					<td>
						...
						<el-select filterable remote clearable :loading="loading" :remote-method="remoteMethod" v-model="data2.relatedDiseases"
						 placeholder="请选择">
							<el-option v-for="(item, index) in diseaseListEn" :key="index" :label="item.value" :value="item.name">
							</el-option>
						</el-select>
					</td>
	...
	export default {
		...
		methods: {
					...
					if (res.data.code === '100') {
						...
						for (let i = 0; i < that.diseaseList.length; i++) {
							that.diseaseListEn.push({
								value: that.diseaseList[i].diseaseEnName + "(" + that.diseaseList[i].name + ')',
								name: that.diseaseList[i].diseaseEnName
							})
						}
					}
				}).catch(err => {
			...
			save(val) {
				...
				if(val == 3 && that.pstatus == '1000') {
					url = 'cloud/knowledge/pharmgkbAnnotaion/disableByCode'
				}
