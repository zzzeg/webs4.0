<template>
  <div class="base_msg blockContent">
    <div class="subItems">
      ...
      <div class="addBar" style="padding:0px 55px 10px 0;margin: -43px 0 3px 0; text-align:right;">
        <el-button type="primary" size="medium" @click="goAdd()">添加</el-button>
        <!-- <el-button type="primary" @click="">发送邮件</el-button> -->
      </div>
      <div class="">
        <el-table class="tableList hasLine" ref="table" :data="formData1.list" empty-text="未检出相关变异" border stripe style="width: 100%"  @cell-click="cellClick">
              ...
              <template slot-scope="scope">
  	      	...
  	      	<el-table-column prop="variant" label="负荷程度">
              <template slot-scope="scope">
                ...
                <!-- <span v-if="!scope.row.isEdit && !scope.row.isAdd">{{ scope.row.variant }}<span :class="scope.row.hasValidata == '1' ? 'yellosj' :'bluesj'"></span><span v-if="scope.row.checkCell == 'variant'" class="allBg"></span></span> -->
              </template>
  	      	</el-table-column>
		...
		<div class="notes">
			...
			<p>1、TMB-H &nbsp;&nbsp;肿瘤突变负荷高，TMB≥20个/Mb。</p>
			<p>2、TMB-M &nbsp;&nbsp;肿瘤突变负荷中等，6≤TMB<20个/Mb。</p>
			<p>3、TMB-L &nbsp;&nbsp;肿瘤突变负荷低，TMB<6个/Mb。</p>
		</div>
  </div>
</template>
<script>
...
//          "transcriptExon": "",
        ...
        that.formData1.list.splice(0, 0, needAddsList)
      },
      pushListItem(type, code) {
        ...
        if (type == 'add') {
          if (that.formData1.list[0]['gene'] == '' || that.formData1.list[0]['variant'] == '') {
            ...
            that.needAddsList['gene'] = that.formData1.list[0]['gene']
            that.needAddsList['variant'] = that.formData1.list[0]['variant']
            that.needAddsList['mutationalLoad'] = that.formData1.list[0]['mutationalLoad']
