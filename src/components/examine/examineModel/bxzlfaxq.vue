		...
		<draggable v-else handle=".handle" :options="{group:'people',animation:150,ghostClass:'sortable-ghost',chosenClass:'chosenClass',scroll:true,scrollSensitivity:200}"
			...
			<div class="details_cont" v-for="(formList, index) in formData1" :key="index">
				...
				<div class="details_content" v-show="formList.isOpen">
					<el-row v-if="formList.drugList && formList.drugList.length > 0">
					...
					<div class="xxxxx" style="border: 1px solid #D9DDDF; margin-bottom:-6px;" v-if="formData.blockCode == '66' || formData.blockCode == '57'">
						<el-row>
							<el-col style="width:5em;">结论：</el-col>
							<el-col :span="22" >
								<div class="tableListItem" v-if="!formList.isEdit">{{ formList.conclusion }}</div>
								<el-input type="textarea" v-model="formList.conclusion" :autosize="{ minRows: 1, maxRows: 2}" v-if="formList.isEdit" placeholder="结论"></el-input>
							</el-col>
						</el-row>
					</div>
					<div class="content_textarea" v-for="(noteList, i) in formList.reportTherapyVariantCancerClinicalNoteList" :key="i">
						<div class="content_input">
							...
							<el-row>
								...
								<!--<el-col :span="2" v-if="formData.blockCode!= 57">影响类型：</el-col>
								<el-col :span="2" v-if="formData.blockCode!= 57">
	...
	export default {
		...
		methods: {
			...
			saveChange(vale, index) { // 保存
				...
				n['code'] = vale.code
				if(that.formData.blockCode == '66' || that.formData.blockCode == '57') {
					n['conclusion'] = vale.conclusion
				}
