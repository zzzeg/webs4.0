			...
			<div class="">
				<el-table class="tableList hasLine" :data="formData1.list" empty-text="未检出相关变异" border stripe style="width: 100%" ref="table" @cell-click="cellClick">
					...
					<el-table-column prop="fusionType" label="融合类型">
						<template slot-scope="scope">
							<span v-if="!scope.row.isEdit && !scope.row.isAdd">{{ scope.row.fusionType }}</span>
							 <el-input v-if="scope.row.isEdit || scope.row.isAdd" v-model="scope.row.fusionType" placeholder=""></el-input>
						</template>
					</el-table-column>
			...
			pushListItem(type, code) {
				...
				if (type == 'add') {
					if (that.formData1.list[0]['gene'] == '' || that.formData1.list[0]['variant'] == '') {
			            ...
			            that.needAddsList['fusionType'] = that.formData1.list[0]['fusionType']
