<template>
	<div class="lists">
		<div class="contentTitle pdlr20">添加审核知识库</div>
		<div class="knowledge_tab" style="display:inline-block; vertical-align:middle; margin:20px 0px 20px 20px;">
			<span @click.stop="changeTabId(1)" :class="{'active':showTab === 1}">实体</span>
			<span @click.stop="changeTabId(2)" :class="{'active':showTab === 2}">标注</span>
			<span @click.stop="changeTabId(3)" :class="{'active':showTab === 3}">结论</span>
		</div>
		<!-- 实体 -->
		<div class="examinedatainfo" v-show="showTab === 1">
			<table>
				...
				<tr>
					...
					<td>
						<el-select multiple v-model="data2.variantTypes" @visible-change="(val) => pushVariantType(val)" placeholder="请选择">
							<!-- <el-option v-for="(item, index) in mutationList" :key="index" :label="item.label" :value="item.value"></el-option> -->
						</el-select>
					</td>
				</tr>
				<tr>
					<td class="texts">药物名(英文)</td>
					<td>
				...
				<tr>
					...
					<td>
						<el-select v-model="data2.drugType" placeholder="请选择">
							<el-option v-for="(item, index) in drugTypeList" :key="index" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<el-select filterable clearable @change="(val)=>handleSelect('en', val)" v-model="data2.diseaseEnName"
						 placeholder="请选择">
							<el-option v-for="(item, index) in diseaseList" :key="index" :label="item.diseaseEnName" :value="item.diseaseEnName"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select filterable clearable @change="(val)=>handleSelect('cn', val)" v-model="data2.diseaseName" placeholder="请选择">
							<el-option v-for="(item, index) in diseaseList" :key="index" :label="item.name" :value="item.name"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.toxincityRisk" placeholder="请选择">
							<el-option v-for="item in duxingrisk" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.drugReaction" placeholder="请选择">
							<el-option v-for="item in yaowufanying" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.metabolism" placeholder="请选择">
							<el-option v-for="item in daixie" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.absorption" placeholder="请选择">
							<el-option v-for="item in xishou" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.prognosis" placeholder="请选择">
							<el-option v-for="item in yuhou" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.dosage" placeholder="请选择">
							<el-option v-for="item in duxing" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.sideEffect" placeholder="请选择">
							<el-option v-for="item in fuzuoyong" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select clearable v-model="data2.sideEffectRisk" placeholder="请选择">
							<el-option v-for="item in fuzuoyongRisk" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select multiple v-model="data2.treatmentStages" placeholder="请选择">
							<el-option v-for="item in zhiliaojieduan" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<el-select multiple v-model="data2.therapys" placeholder="请选择">
							<el-option v-for="item in therapysDownList" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</td>
				</tr>
				<tr>
       ...
       <!-- <tr>
          <td class="texts">肿瘤免疫超进展</td>
          <td>
            <el-select clearable v-model="data2.advancesInTumor" placeholder="请选择">
              <el-option v-for="item in advancesInTumor" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </td>
        </tr>-->
				<tr>
						...
						<div style="margin:10px;" v-for="(obj, index) in data2.conclusions" :key="index">
							...
							<div class="source">
								...
								<span style="display:inline-block;width:10em;">
									<el-select v-model="obj.key" placeholder="证据来源" @change="(val)=>cancerReferList(val, index)">
							...
							<p>结论内容：</p>
							<el-input type="textarea" :autosize="{ minRows: 2, maxRows: 8}" id="male" placeholder="请输入结论内容" v-model="obj.value"></el-input>
							<p>参考文献：</p>
							<el-input type="textarea" :autosize="{ minRows: 2, maxRows: 8}" placeholder="例子:NCCN Clinical Practice Guidelines in Oncology_Melanoma(Version 2.2018)|||http://www.nccn.org/patients;^PMID$ 26037941|||Long G V, Stroyakovskiy D, Gogas H, et al. Dabrafenib and trametinib versus dabrafenib and placebo for Val600 BRAF-mutant melanoma: a multicentre, double-blind, phase 3 randomised controlled trial[J]. Lancet, 2015, 386(9992):444-451."
							 autosize v-model.trim="obj.ref"></el-input>
							<p>依&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;据：</p>
		...
		<!-- 底部按钮 -->
		<div class="footer_btn" style="text-align: center;margin: 30px 0 20px;">
			...
			<el-button type="primary" size="medium" @click="showDialogFun('sub')">提交</el-button>
			<!-- <el-button type="warning" size="medium" @click="subNext('')">保存</el-button> -->
	...
	export default {
		...
		data() {
			return {
				...
				reportVariantCode: this.$route.params.reportVariantCode,
		...
		methods: {
						...
						for (let key in that.data1) {
							if (Array.isArray(that.data1[key])) {
								...
								for (let i = 0; i < that.data1[key].length; i++) {
									that.data2[key][i] = JSON.parse(JSON.stringify(that.data1[key][i]))
								}
							} else {
			...
			getInitData() {
				...
				let that = this
				that.loading = true
				axios.get(URL.api_name + 'cloud/project/getReportVariantByCode', {
					params: {
						code: that.reportVariantCode,
					}
				}).then(function(respose) {
					let data = respose.data
					that.data2.gene = data.data.gene
					that.data2.variant = data.data.variant
					that.data2.rsId = data.data.rsId
					that.loading = false
				}, function(error) {
					// console.log(error)
					that.loading = false
			...
			getDrugList() {
				...
				axios.get(URL.api_name + 'report/getDrugControlList', {
						...
						if (respose.data.code === '100') {
							that.drugTypeListee = respose.data.data.list
			...
			handleSelectDrug(language, val) {
				...
				console.log(that.data2.drugType + 'e')
				console.log(that.drugTypeListee[0].drugType + 'c')
				// 对比药物英文名，然后赋值药物类型
				for (let i = 0; i < that.drugTypeListee.length; i++) {
					if (val == that.drugTypeListee[i].drugEnName) {
						that.data2.drugType = that.drugTypeListee[i].drugType
					}
				}
				console.log(that.data2.drugType + 'xxx')
			},
			...
			contrast() {
				...
				for (let i in that.data1) {
					// if (Array.isArray(that.data1[i])) {
					//   for (let n = 0; n < that.data1[i].length; n++) {
					//     if(that.data2[i][n] = that.data1[i][n]) {
