  ...
  <div class="list">
    ...
    <div class="search-wrapper">
      <el-form :inline="true" :model="formInline" class="demo-form-inline query-form">
        ...
        <el-form-item>
          ...
          <el-button type="primary" size="small" @click="goSend()">发送邮件</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="table-data">
      <el-table :data="tableData" v-loading="false" border style="width: 100%">
        ...
        <el-table-column prop="code" label="标题"></el-table-column>
        <el-table-column prop="applyName" label="发件人"></el-table-column>
        <el-table-column prop="applyPhone" label="收件人"></el-table-column>
        <el-table-column prop="applyPhone" label="抄送人"></el-table-column>
        <!-- <el-table-column prop="lastUpdateTime" :formatter="dateFormat" label="最后编辑时间"></el-table-column> -->
        <el-table-column prop="status" label="状态">
          <template slot-scope="scope">
            {{ scope.row.status === 1 ? '已发送' : scope.row.status === 3 ? '草稿' : '状态出错'}}
          </template>
        </el-table-column>
        <el-table-column label="操作" width="140">
          <template slot-scope="scope">
            <!-- <el-button type="primary" size="mini" v-if="scope.row.status === 1" @click="goinfo(scope.row.id)">审核</el-button> -->
            <el-button type="primary" size="mini" @click="goDetail(scope.row.orderId)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
