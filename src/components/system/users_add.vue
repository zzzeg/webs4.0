<template>
  <div class="list">
    <div class="contentTitle pdlr20">添加用户</div>
        ...
        <el-form-item label="性别：">
          <el-col :span="9">
            <!--<el-select v-model="formData.sex" clearable placeholder="请选择">-->
              <!--<el-option :label="item.label" :value="item.value" v-for="item in sexs" :key="item.id"></el-option>-->
            <!--</el-select>-->
    ...
    methods: {
      ...
      sub (formName) {
        ...
        that.$refs[formName].validate((valid) => {
          if (valid) {
            // 如果验证通过，即提交
            axios.post(URL.api_name + 'cloud/user_add', that.formData).then((res) => {
