		...
		<div class="table-data" v-loading="loading" element-loading-text="拼命加载中">
			<el-table :data="tableData" style="width: 100%" stripe>
				...
				<el-table-column label="操作" width="150">
					<template slot-scope="scope">
						<el-button type="text" size="mini" @click="goNext(scope.row, scope.row.productCode,scope.row.productType)">创建</el-button>
					</template>
				</el-table-column>
			</el-table>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goNext(scope, productCode, productType) {
				let that = this
				if (that.serviceCreatType == 'creat') {
					this.$router.push({
						path: '/ServiceAddCode/' + scope.code + '/' + productCode + '/' + that.currentPage + '/' + productType
					})
				} else if (that.serviceCreatType == 'receive') {
					// 跳转到编辑
					this.$router.push({
						path: '/serviceEdit/' + scope.code + '/' + that.pcode + '/' + that.currentPage + '/' + productCode + '/' + productType
					})
				}
