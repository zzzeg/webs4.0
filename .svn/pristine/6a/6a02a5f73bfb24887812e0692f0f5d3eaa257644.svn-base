<template>
	<div class="list">
		<div class="contentTitle pdlr20">药物添加</div>
		<div class="table-data" v-loading="loading" element-loading-text="拼命加载中">
			<el-form :model="formData" :rules="rules" ref="formData" label-width="115px">
				<el-form-item label="药物英文名：" prop="drugName">
					<el-col :span="6">
				...
				<!-- <el-form-item label="药物商品英文名：">
          <el-col :span="6">
            <el-input v-model="formData.drugTradeEnName"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="药物商品中文名：">
          <el-col :span="6">
            <el-input v-model="formData.drugTradeName"></el-input>
          </el-col>
        </el-form-item> -->
				...
				<el-form-item label="备注：" v-if="formData.healthInsurance == '是'">
					<el-col :span="12">
		...
		methods: {
			...
			sub(formName) {
				...
				that.$refs[formName].validate((valid) => {
					if (valid) {
						axios.post(URL.api_name + 'report/addDrugControl', that.formData).then((res) => {
							that.isLoading = false
							if (res.data.code === '100') {
								that.$message({
