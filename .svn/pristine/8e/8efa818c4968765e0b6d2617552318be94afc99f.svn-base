<template>
  <div class="list">
    ...
    <div class="table-data" v-loading="loading" element-loading-text="拼命加载中">
      <el-form :model="formData" :rules="rules" ref="formData" label-width="115px">
        <el-form-item label="基因：" prop="gene">
          <el-col :span="4">
            <el-input v-model="formData.gene" placeholder="请输入基因名"></el-input>
          </el-col>
        </el-form-item>

        <el-form-item label="变异：" v-if="status == '2'" :prop="status == '2' ? 'variant' : ''">
          <el-col :span="6">
        ...
        <el-form-item label="疾病相关：" v-if="status == '2'" :prop="status == '2' ? 'pathogenesis' : ''">
          <el-col :span="6">
    ...
    data () {
      return {
        ...
        rules: {
          ...
          variant: [
            { required: true, message: '请输入变异名称', trigger: 'blur' }
          ],
          pathogenesis: [
            { required: true, message: '请选择疾病相关', trigger: 'change' }
          ],
        },
    ...
    methods: {
      getData() {
        ...
        axios.get(URL.api_name + that.getDataurl, {
          ...
          if(respose.data.code === '100' && respose.data.data) {
      ...
      back() {
        this.$router.push({
          path: '/ExamineNotes/' + this.pager + '/' + this.status
        })
      },
      goBack() {//返回按钮
        this.$router.go(-1);
