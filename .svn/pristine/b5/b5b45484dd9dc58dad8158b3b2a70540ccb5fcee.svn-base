      ...
      <el-table class="tableList hasLine" :data="formData1" v-if="isBlood != 0" empty-text="未检测到相关结果" stripe style="width: 100%" ref="table"  @cell-click="cellClick">
        ...
        <el-table-column label="FDA/CFDA/NCCN/CSCO">
          <el-table-column label="本癌种">
            <template slot-scope="scope">
              <div :class="{'drugLine color_red': item.drugReactionType == 1, 'drugLine' : item.drugReactionType == 0 }" style="position:relative;" v-if=" scope.row.fcncThisCancerDrugList.length>0" v-for="(item,index) in scope.row.fcncThisCancerDrugList" :key="index" @click.stop="diseaseClick(scope.row, item.reportTherapy.drugName, item.reportTherapy.drugEnName)"><span>{{ item.reportTherapy.drugName != "" ? item.reportTherapy.drugName : "/"}}<span class="bluesj" style="top: 5px;right: 5px;"></span><span v-if="scope.row.checkCell == 'xxx'" class="allBg"></span></span></div>
          ...
          <el-table-column label="其他癌种">
            <template slot-scope="scope">
              <div class="drugLine" style="position:relative;" v-if="!scope.row.isEdit&&scope.row.fcncOtherCancerDrugList.length!=0" v-for="(item,index) in scope.row.fcncOtherCancerDrugList" :key="index" @click.stop="diseaseClick(scope.row, item.reportTherapy.drugName, item.reportTherapy.drugEnName)"><span>{{ item.reportTherapy.drugName}}({{ item.cancer.name != "" ? item.cancer.name : "/" }})<span class="bluesj" style="top: 5px;right: 5px;"></span><span v-if="scope.row.checkCell == 'xxx'" class="allBg"></span></span></div>
        ...
        <el-table-column label="潜在临床试验（敏感性，适应症）">
          <el-table-column label="本癌种">
            <template slot-scope="scope">
              <div :class="{'drugLine color_red': item.drugReactionType == 1, 'drugLine' : item.drugReactionType == 0 }" style="position:relative;" v-if="!scope.row.isEdit && scope.row.clinicThisCancerDrugList.length!=0" v-for="(item,index) in scope.row.clinicThisCancerDrugList" :key="index" @click.stop="diseaseClick(scope.row, item.reportTherapy.drugName, item.reportTherapy.drugEnName)"><span>{{ item.reportTherapy.drugName}}<span class="bluesj" style="top: 5px;right: 5px;"></span><span v-if="scope.row.checkCell == 'xxx'" class="allBg"></span></span></div>
              <div class="drugLine" v-if="scope.row.clinicThisCancerDrugList && scope.row.clinicThisCancerDrugList < 1">/</div>
            </template>
          </el-table-column>
          <el-table-column label="其他癌种">
            <template slot-scope="scope">
              <div class="drugLine" style="position:relative;" v-if="!scope.row.isEdit && scope.row.clinicOtherCancerDrugList.length!=0" v-for="(item,index) in scope.row.clinicOtherCancerDrugList" :key="index" @click.stop="diseaseClick(scope.row, item.reportTherapy.drugName, item.reportTherapy.drugEnName)"><span>{{ item.reportTherapy.drugName}}({{ item.cancer.name }})<span class="bluesj" style="top: 5px;right: 5px;"></span><span v-if="scope.row.checkCell == 'xxx'" class="allBg"></span></span></div>
