			...
			<div class="tableBig">
				<el-table class="tableList hasLine" ref="table" :data="formData1.list" border stripe style="width: 100%" height="500"
				 @sort-change="sortChange" @cell-click="cellClick">
					...
					<el-table-column prop="transcript_exon" label="外显子" width="120" sortable="custom">
						<template slot-scope="scope">
              ...
              <el-select v-if="scope.row.isEdit || scope.row.isAdd" clearable v-model="scope.row.variantClassification" placeholder="">
                <el-option :label="items.label" :value="items.value" v-for="(items, i) in mutationTypesList" :key="i"></el-option>
              </el-select>
						</template>
					</el-table-column>
					...
					<el-table-column prop="rsId" label="RS_ID" width="100">
						<template slot-scope="scope">
							<span v-if="!scope.row.isAdd&&scope.row.rsId!=''"><a :href="'https://www.ncbi.nlm.nih.gov/clinvar/?term=rs' + scope.row.rsId.split('rs')[1]" target="_blank"> {{ scope.row.rsId }}</a></span>
					...
					<el-table-column prop="clinvarDiseaseName" label="RS_ID疾病" width="260">
						<template slot-scope="scope">
							<span v-if="!scope.row.isAdd">{{ scope.row.clinvarDiseaseName }}</span>
							<el-input v-if="scope.row.isAdd" v-model="scope.row.clinvarDiseaseName" placeholder=""></el-input>
						</template>
					</el-table-column>
					...
					<el-table-column type="expand" width="1">
				       <template slot-scope="props">
				         <popdialog :text="pvalue" :scope="row" :windowType="property" :popData="popData1" :formDatas="formData1"
							 :minMark="minMark" :formInline="formInline" :checkMark="checkMark" :cacheData="cacheData" @cmmk="cmmk" @sWindow="sWindow(property, row.code)" @pushHead="pushHead"
	...
	export default {
		...
		methods: {
			...
			goAdd() {
				...
				that.formData1.list.splice(0, 0, n)
			},
            ...
            that.needAddsList=that.formData1.list[0]
