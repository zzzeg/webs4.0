  ...
  <div class="base_msg blockContent">
    <div class="subItems">
      ...
      <el-table class="tableList hasLine" :data="formData1" stripe style="width: 100%" height="600" ref="table" @cell-click="cellClick">
        ...
        <el-table-column prop="variant" label="检测结果" width="300">
          <template slot-scope="scope">
            <span>{{ scope.row.reportVariant.variant }}</span>
        ...
        <el-table-column label="有效性" width="200">
          <template slot-scope="scope">
            <div class="drugLine" v-if="!scope.row.isEdit" v-for="(item,index) in scope.row.reportTherapyEfficacyList" :key="index">{{ item.drugReactionType == '0' ? '有效性可能提高' : item.drugReactionType == '8' ? '肿瘤免疫超进展': '有效性可能降低'}}</div>
