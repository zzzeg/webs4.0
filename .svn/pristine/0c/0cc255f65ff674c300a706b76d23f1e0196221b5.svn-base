		...
		<div class="search-wrapper">
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				...
				<el-form-item label="产品名称">
					<el-input v-model="formInline.productName" size="medium" placeholder="请输入检测项目" clearable></el-input>
		...
		<div class="table-data" v-loading="loading" element-loading-text="拼命加载中">
			<el-table :data="tableData" style="width: 100%" stripe>
				...
				<el-table-column prop="status" label="状态" width="130" sortable>
					<template slot-scope="scope">
						{{ scope.row.status == 1 ? '草稿' : scope.row.status == 2 ? '运行中...' : scope.row.status == 3 ? '未审核' : scope.row.status == 4 ? '解读完成' : scope.row.status == 6 ? '分析流程异常': scope.row.status == 66 ? '后台流程异常' : scope.row.status == 96 ? '审核中' : scope.row.status == 11 ? '未审核' : scope.row.status == 100 ? '订单完成' : scope.row.status == 60 ? '已收样' : scope.row.status == 65 ? '已检测' : '已删除' }}
					</template>
				</el-table-column>
				<el-table-column label="操作" width="230">
					<template slot-scope="scope">
						<el-button type="text" v-if="scope.row.businessCode == '' && scope.row.status != 99 && scope.row.status == 3 " @click="gopush(scope.row)">接收</el-button>
            <el-button type="text" v-if="scope.row.businessCode !== '' && scope.row.systemSource!=3 && scope.row.productType == 3 && scope.row.createUserCode == '' && scope.row.status == 3 " @click="goEdit(scope.row, scope.row.businessCode, scope.row.code, currentPage, scope.row.productCode, scope.row.productType)">接收</el-button>

						<el-button type="text" v-if="scope.row.status == 1 || scope.row.status == 6" @click="goEdit(scope.row, scope.row.businessCode, scope.row.code, currentPage, scope.row.productCode, scope.row.productType)">修改</el-button>
						<el-button type="text" v-if="scope.row.status == 1 || scope.row.status == 6" @click="goDel(scope.row.code)">删除</el-button>
						<el-button type="text" v-if="scope.row.status == 1 || scope.row.status == 6" :loading="btnloading" @click="goRun(scope.row.code)">生成报告</el-button>
	...
	export default {
		...
		methods: {
			...
			goEdit(scope, id, typeid, pager, productCode, productType) {
				...
				localStorage.setItem('serviceCreatType', 'edit')
