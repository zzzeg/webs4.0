<template>
	<div class="base_msg blockContent">
		<div class="subItems">
			<div class="subname" style="padding: 0 88px 10px 0;">
				<span>BRCA参考文献</span>
					...
					<el-table :data="formData1.list" style="width: 100%" stripe border>
						...
						<el-table-column prop="status" label="操作" width="120">
							<template slot-scope="scope">
								...
								<el-button type="text" size="medium" v-if="scope.row.isAdd || scope.row.isEdit" @click="saveChange(scope.row, scope.row.code)"><i class="el-icon-circle-check-outline" style="font-size: 18px;"></i></el-button>
...
<script>
	export default {
		...
		methods: {
			...
			goAdd() {//添加
			  let that = this
			  let brcalist = {code: '', name: '', url: '', isAdd:true}
			  that.formData1.list.splice(0, 0, brcalist)//第一个参数位置，第二个参数添加几个
			},
