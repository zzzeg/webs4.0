<template>
	<div class="base_msg">
		...
		<div class="addBtn" style="text-align: right; position:relative; z-index:2;">
			<el-button style="position:absolute; z-index:4; top:0; right:115px;" type="warning" size="medium" v-if="tabname == 'two'" @click="opendruglist = !opendruglist,opendruglistdel()">删除用药列表</el-button>
		...
		<el-tabs type="border-card" v-model="tabname" style="position:relative; z-index:1;" @tab-click="tabclick">
			...
			<el-tab-pane label="推荐用药" name='two'>
				<div class="tableList" v-loading="loading" element-loading-text="拼命加载中">
					<el-table :data="tabData2" style="width: 100%" emptyText="无相关用药推荐" stripe row-key="code" height="600" ref="table">
						...
						<el-table-column prop="drugName" label="药物名称" align="center">
							<template slot-scope="scope">
				              <div style="position:relative;" @click.stop="diseaseClick(scope.row, scope.row.drugName, scope.row.drugEnName)"><span>{{ scope.row.drugName}}<span class="bluesj" style="top: -5px;right: 5px;"></span><span v-if="scope.row.checkCell == 'xxx'" class="allBg"></span></span></div>
				            </template>
						</el-table-column>
						...
						<el-table-column type="expand" width="1">
					       <template slot-scope="props">
					         <popdialog :text="pvalue" :scope="row" :windowType="property" :popData="popData1" :formDatas="tabData2"
				...
				<div class="details_cont" v-else empty-text="未检测到相关结果" v-for="(formList, index) in tabData3" :key="index">
					<div class="details_title">
						<div class="sleft" slot="reference">
							<div @click="sort('tabData3', index)" style="margin-right:5px;"><i class="el-icon-caret-right" :style="'font-size:18px;transform:rotate(' + (formList.hide ? '90' : '0') + 'deg)'"></i></div>
					...
					<div class="details_content" v-show="tabData3[index].hide">
						<el-row>
			...
			<el-tab-pane label="免疫" name='four'>
		        <div class="noContentsDiv" v-if="tabData4.length == 0">
		          暂无对药物影响的报道。
		        </div>
				<div class="details_cont" v-else v-for="(formList, index) in tabData4" :key="index">
					<div class="details_title">
						<div class="sleft" slot="reference">
							<div @click="sort('tabData4', index)" style="margin-right:5px;"><i class="el-icon-caret-right" :style="'font-size:18px;transform:rotate(' + (formList.hide ? '90' : '0') + 'deg)'"></i></div>
							<div class="textfa">{{index + 1}} 、{{formList.schemeType == '1' ? '治疗方案' : '潜在位点临床用药'}}</div>
							<div class="dunhao" v-for="(drugsele, index) in formList.drugList" :key="index" @click="sWindow('drug', drugsele.drugEnName)">
								{{drugsele.drugName == '' ? drugsele.drugEnName : drugsele.drugName}}</div>
						</div>
					...
					<div class="details_content" v-show="tabData4[index].hide">
						<el-row>
							<!-- <el-col :span="2">治疗阶段：</el-col>
							<el-col :span="6">
								<div class="tableListItem">{{formList.treatmentStage}}</div>
							...
							<el-col :span="2">疗效：</el-col>
							<el-col :span="6">
								<!--<div class="tableListItem">{{formList.drugReactionType == '0' ? '敏感':formList.drugReactionType == '8' ? '肿瘤免疫超进展' : '耐药'}}</div>-->
			...
			<el-tab-pane label="化疗" name='five'>
		        <div class="noContentsDiv" v-if="tabData5.length == 0">
		          暂无对药物影响的报道。
		        </div>
				<div class="details_cont" v-else v-for="(formList, index) in tabData5" :key="index">
					<div class="details_title">
						<div class="sleft" slot="reference">
							<div @click="sort('tabData5', index)" style="margin-right:5px;"><i class="el-icon-caret-right" :style="'font-size:18px;transform:rotate(' + (formList.hide ? '90' : '0') + 'deg)'"></i></div>
							<div class="textfa">{{index + 1}} 、{{formList.schemeType == '1' ? '治疗方案' : '潜在位点临床用药'}}</div>
							<div class="dunhao" v-for="(drugsele, index) in formList.drugList" :key="index" @click="sWindow('drug', drugsele.drugEnName)">
								{{drugsele.drugName == '' ? drugsele.drugEnName : drugsele.drugName}}</div>
						</div>
					...
					<div class="details_content" v-show="tabData5[index].hide">
						<el-row>
							<el-col :span="2">分型：</el-col>
							<el-col :span="6">
								<div class="tableListItem">{{formList.cancer ? formList.cancer.name : ''}}</div>
		...
		<!---->
	    <el-dialog title="已删除推荐用药列表" :visible.sync="opendruglist" width="80%">
	      <el-table :data="syncData">
	        ...
	        <el-table-column label="操作" width="100">
	            <template slot-scope="scope">
	              <el-button type="success" size="small" @click="huanyuan(scope.row.code)">还原</el-button>
	            </template>
          	</el-table-column>
	      </el-table>
	    </el-dialog>
...
<script>
	...
	export default {
		...
		methods: {
			huanyuan(code) {
				let that = this
				axios.post(URL.api_name + 'report/therapy/recommended/reduction', {
					code: code
				}).then(function(res){
			...
			opendruglistdel() {
				let that = this
				// 查询已删除推荐用药列表
				axios.get(URL.api_name + 'report/therapy/recommendedDelDrugList', {
		          params: {
		          	projectCode: that.code
		          }
		        }).then((res) => {
		        	if(res.data.code == '100') {
		        		that.syncData = res.data.data
		        	}
		        }).catch((error)=> {
		        	// 接口报错了
		        })
			...
			sort(tableData, index) {
				let that = this
				that.$set(that[tableData][index], 'hide', that[tableData][index].hide ? false : true)
				// that[tableData][index].hide = true
				console.log(that[tableData][index].hide)
			},
			...
			tabclick(tab, event) { //获取数据
				...
				if (a == 'one') {
					...
					that.rowDrop() // 排序调用，返回太慢了，只能在用之前调用
				} else if (a == 'three') {
...
.el-table__expanded-cell{ padding: 0!important; }
</style>
