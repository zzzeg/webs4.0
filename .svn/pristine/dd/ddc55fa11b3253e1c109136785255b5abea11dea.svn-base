<template>
	<div class="base_msg blockContent">
		<div class="subItems">
			...
			<div class="">
				<el-table class="tableList hasLine" :data="formData1.list" empty-text="未检出相关变异" border stripe style="width: 100%" ref="table" @cell-click="cellClick">
					...
					<el-table-column prop="variant" label="变异类型">
						<template slot-scope="scope">
							<span v-if="!scope.row.isEdit && !scope.row.isAdd">{{ scope.row.variant }}
              <span class="bluesj"></span><span v-if="scope.row.checkCell == 'variant'" class="allBg"></span></span>
					...
					<el-table-column prop="copyNumber" label="拷贝数">
              <template slot-scope="scope">
                <span v-if="!scope.row.isEdit && !scope.row.isAdd">{{ scope.row.copyNumber }}</span>
                <el-input v-if="scope.row.isAdd || scope.row.isEdit" v-model.number="scope.row.copyNumber" placeholder=""></el-input>
              </template>
            </el-table-column>
...
//					"transcriptExon": "",
            ...
            that.needAddsList['copyNumber'] = that.formData1.list[0]['copyNumber']
