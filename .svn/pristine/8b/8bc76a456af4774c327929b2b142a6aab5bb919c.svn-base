<template>
	<div class="base_msg">
		<div class="subtable dtitleBg">靶向治疗方案详情</div>
    ...
    <div class="noContentsDiv" v-if="formData1.length == 0">
      暂无对药物影响的报道。
		...
		<div class="details_cont" v-else v-for="(formList, index) in formData1" :key="index">
			<div class="details_title">
				<div class="sleft">
					...
					<div class="btn" style="margin:5px 5px 0 0; float:right">
        ...
        <el-row>
          ...
          <el-col :span="21" v-for="(drugPub, n) in formList.drugList" :key="n" v-if="drugPub.inlandPublicSituation != ''">
            <!-- <div class="tableListItem">{{drugPub.inlandPublicSituation}}</div> -->
            <div class="tableListItem">{{drugPub.drugName!='' ? (drugPub.drugName + '：' + drugPub.inlandPublicSituation) : (drugPub.drugEnName + '：' + drugPub.inlandPublicSituation)}}</div>
          </el-col>
        </el-row>
					...
					<div class="content_input">
						<el-row>
							...
							<el-col :span="2" style="position: absolute;right: -49px;" class="deletbtn">
								<el-button icon="el-icon-delete" circle v-if="formList.isEdit" size="small" @click="deleteTodo(index, i)"></el-button>
							</el-col>
						</el-row>
						<el-row>
...
//								that.blockSearch(formInline)
			...
			deleteTodo(index, i) {
				...
				that.formData[index].reportTherapyVariantCancerClinicalNoteList.splice(i, 1)
			},
		    addckwx (index, i, q) {
		    	// 添加参考文献
		    	let that = this
